...
def model_to_flags(m: ModelRef) -> tuple[str, str]:
    ...
    return ("0" if not n else " | ".join(n), is_folder_value)
...
def open_create_flags():
    ...
    grd13 = And(
        Not(And(rdonly, wronly)), Not(And(rdonly, rdwr)), Not(And(wronly, rdwr))
    )
    ...
    return solve(
        "open_create",
        {
            ...
            "grd12[FFF]": And(
                grd11, Not(rdonly), Not(wronly), Not(rdwr), grd13, grd14, grd18
            ),
            "grd12[TFF]": And(
                grd11, rdonly, Not(wronly), Not(rdwr), grd13, grd14, grd18
            ),
            "grd12[FTF]": And(
                grd11, Not(rdonly), wronly, Not(rdwr), grd13, grd14, grd18
            ),
            "grd12[FFT]": And(
                grd11, Not(rdonly), Not(wronly), rdwr, grd13, grd14, grd18
            ),
            "grd14[FFF]": And(
                grd11, grd12, grd13, Not(Not(tmpfile)), Not(wronly), Not(rdwr), grd18
            ),
            "grd14[TFF]": And(
                grd11, grd12, grd13, Not(tmpfile), Not(wronly), Not(rdwr), grd18
            ),
            "grd14[FTF]": And(
                grd11, grd12, grd13, Not(Not(tmpfile)), wronly, Not(rdwr), grd18
            ),
            "grd14[FFT]": And(
                grd11, grd12, grd13, Not(Not(tmpfile)), Not(wronly), rdwr, grd18
            ),
...
def open_create_test_function():
    ...
    return textwrap.dedent(f"""
        ...
        "".join(
            [
                nl + f'                child.open("/file{i}", {f[0]}, 0o777)'
...
def openat_create_test_function():
    ...
    return textwrap.dedent(f"""
        ...
        "".join(
            [
                nl + f'                child.openat(fd, "file{i}", {f[0]}, 0o777)'
                for i, f in enumerate(flags)
            ]
        )
    }
...
def open_exists_test_function():
    ...
    return textwrap.dedent(f"""
        ...
        "".join(
            [
                nl + f'            t.make_{f[1]}("/{f[1]}{i}", "root", "root", 0o777)'
                for i, f in enumerate(flags)
            ]
        )
    }
...
def openat_exists_test_function():
    ...
    return textwrap.dedent(f"""
        ...
        "".join(
            [
                nl
                + f'            t.make_{f[1]}("/dir/{f[1]}{i}", "root", "root", 0o777)'
                for i, f in enumerate(flags)
            ]
        )
    }
        ...
        "".join(
            [
                nl + f'                child.openat(fd, "{f[1]}{i}", {f[0]}, 0)'
                for i, f in enumerate(flags)
            ]
        )
    }
