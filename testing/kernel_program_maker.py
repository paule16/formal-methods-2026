...
def mode2expr(mode: int) -> Expression:
    return Expression(
        f"0{(mode & 0o777):0o}"
        + "".join([f"| {ModeBits[m]}" for m in ModeBits if (mode & m) != 0])
    )
...
class KernelProgramMaker(ProgramMakerTextProducer):
    ...
    def bound_value_as_uid_t(
        self, init: Optional[Expression] = None, prefix: str = "uid"
    ):
    ...
    def bound_value_as_charparray(
        self, init: Optional[List[str]] = None, prefix: str = "strings"
    ):
        ...
        return self._bound_value(
            ...
            prefix=prefix,
            init=f" = {{{''.join([f'{q}{s}{q}, ' for s in init])}NULL}}"
            if init is not None
            else None,
        )

    def bound_value_as_chararray(
        self,
        ...
        return self._bound_value(
            type=lambda string: (
                f"char {string}[{str(size) if size is not None else ''}]"
            ),
            prefix=prefix,
            init=f' = "{init}"' if init is not None else None,
        )
    ...
    def open(self, pathname: str, flags: int, mode: int, fatal: bool = False):
        ...
        if fatal:
            ...
            self.main_lines.append(
                f'{{ {self._make_pathname(pathname)} if (({fd.text} = {syscall}) < 0) {{ perror(__FILE__ ":" LINE_STRING ":" "{pathname}"); return 2; }} }}'
            )

        else:
            self.main_lines.append(
                f"{{ {self._make_pathname(pathname)} {fd.text} = {syscall}; }}"
            )
    ...
    def mkdir(self, pathname: str, mode: int, fatal: bool = False):
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'{{ {self._make_pathname(pathname)} if ({syscall} != 0) {{ perror(__FILE__ ":" LINE_STRING ":" "mkdir"); return 2;}} }}'
            )
    ...
    def chmod(self, pathname: str, mode: int, fatal: bool = False):
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'{{ {self._make_pathname(pathname)} if ({syscall} != 0) {{ perror(__FILE__ ":" LINE_STRING ":" "chmod"); return 2;}} }}'
            )
    ...
    def fchmod(self, fd: Expression, mode: int, fatal: bool = False):
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'if ({syscall} != 0) {{ perror(__FILE__ ":" LINE_STRING ":" "fchmod"); return 2; }}'
            )
    ...
    def chdir(self, pathname: str, fatal: bool = False):
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'{{ {self._make_pathname(pathname)} if ({syscall} != 0) {{ perror(__FILE__ ":" LINE_STRING ":" "chdir"); return 2;}} }}'
            )
    ...
    def fchdir(self, fd: Expression, fatal: bool = False):
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'if ({syscall} != 0) {{ perror(__FILE__ ":" LINE_STRING ":" "fchdir"); return 2; }}'
            )
    ...
    def chown(self, pathname: str, uid: Expression, gid: int, fatal: bool = False):
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'{{ {self._make_pathname(pathname)} if ({syscall} != 0) {{ perror(__FILE__ ":" LINE_STRING ":" "chown"); return 2;}} }}'
            )
    ...
    def fchown(self, fd: Expression, uid: Expression, gid: int, fatal: bool = False):
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'if ({syscall} != 0) {{ perror(__FILE__ ":" LINE_STRING ":" "fchown"); return 2; }}'
            )
    ...
    def getxattr(
        self, path: str, name: str, value: Expression, size: int, fatal: bool = False
    ):
        assert '"' not in path
        assert '"' not in name
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'{{ {self._make_charbuf("path", path)} {self._make_charbuf("name", name)} if ({syscall} != 0) {{ perror(__FILE__ ":" LINE_STRING ":" "getxattr"); return 2;}} }}'
            )

        else:
            self.main_lines.append(
                f"{{ {self._make_charbuf('path', path)} {self._make_charbuf('name', name)} {syscall}; }}"
            )
    ...
    def seteuid(self, euid: int, fatal: bool = False):
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'if ({seteuid_call} != 0) {{ perror(__FILE__ ":" LINE_STRING ":" "seteuid"); return 2; }}'
            )
    ...
    def setxattr(
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'{{ {self._make_charbuf("path", path)} {self._make_charbuf("name", name)} {self._make_charbuf("value_literal", value_literal)} if ({syscall} != 0) {{ perror(__FILE__ ":" LINE_STRING ":" "setxattr"); return 2;}} }}'
            )

        else:
            self.main_lines.append(
                f"{{ {self._make_charbuf('path', path)} {self._make_charbuf('name', name)} {self._make_charbuf('value_literal', value_literal)} {syscall}; }}"
            )
    ...
    def execve(
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'{{ {self._make_charbuf("path", exeFile.path)} {syscall}; perror(__FILE__ ":" LINE_STRING ":" "{exeFile.path}"); return 2; }}'
            )

        else:
            self.main_lines.append(
                f"{{ {self._make_charbuf('path', exeFile.path)} {syscall}; }}"
            )
    ...
    def link(self, oldpath: str, newpath: str, fatal: bool = False):
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'{{ {self._make_charbuf("oldpath", oldpath)} {self._make_charbuf("newpath", newpath)} if ({syscall} != 0) {{ perror(__FILE__ ":" LINE_STRING ":" "link"); return 2;}} }}'
            )

        else:
            self.main_lines.append(
                f"{{ {self._make_charbuf('oldpath', oldpath)} {self._make_charbuf('newpath', newpath)} {syscall}; }}"
            )
    ...
    def unlink(self, pathname: str, fatal: bool = False):
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'{{ {self._make_pathname(pathname)} if ({syscall} != 0) {{ perror(__FILE__ ":" LINE_STRING ":" "unlink"); return 2;}} }}'
            )
    ...
    def rmdir(self, pathname: str, fatal: bool = False):
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'{{ {self._make_pathname(pathname)} if ({syscall} != 0) {{ perror(__FILE__ ":" LINE_STRING ":" "rmdir"); return 2;}} }}'
            )
    ...
    def close(self, fd: Expression, fatal: bool = False):
        ...
        if fatal:
            self.includes.add("#include <stdio.h>")
            self.main_lines.append(
                f'if ({syscall} != 0) {{ perror(__FILE__ ":" LINE_STRING ":" "close"); return 2; }}'
            )
