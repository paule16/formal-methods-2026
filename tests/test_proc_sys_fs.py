...
from tests.methodic.cases import (
    AclCasesGroup,
    UserGroupPair,
    acl_cases_grouped,
    user,
...
@mark.skipif(
    condition=("ProtectedHardlinks" not in link.machines),
    reason="Intended only for ProtectedHarlinks machine",
)
def test_protected_hardlinks(
    t: LinuxTestSpec,
    acl_users: None,
    ...
    for i, case in enumerate(cases):
        ...
        t.add_setup(
            f"setfacl -m {','.join(acl_entries)} /orig{i}"
        )  # test_acl_access() sets acls to /dir, not to /orig
