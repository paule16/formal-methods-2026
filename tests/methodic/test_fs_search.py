...
def pparent_mode(pparent_read: bool, pparent_exec: bool):
    return ",".join(
        ["u+r" if pparent_read else "u-r", "u+x" if pparent_exec else "u-x"]
    )


@fixture
def parent_mode(parent_read: bool, parent_exec: bool):
    return ",".join(["u+r" if parent_read else "u-r", "u+x" if parent_exec else "u-x"])
...
def make_path_and(
    object_kind: ObjectKind, t: LinuxTestSpec, parent_mode: str, pparent_mode: str
) -> str:
    t.make_user("user")
...
def test_chown(
    ...
    with t.make_program_and_run(
        "user", "user", umask=0o022, runner="export USER=$(id -u user); <>"
    ) as child:
