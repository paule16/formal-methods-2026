...
def test_open_file(
    ...
    t: LinuxTestSpec,
    acl_users: None,
    acl_cases_group: Tuple[UserGroupPair, AclCasesGroup],
):
    ...
    for i, case in enumerate(cases):
        ...
        t.make_file(
            f"/file{i}", owner=object_user, group=object_group, mode=object_mode
        )
...
def test_getxattr(
    ...
    for i, case in enumerate(cases):
        ...
        if object_kind == "file":
            t.make_file(
                f"/obj{i}", owner=object_user, group=object_group, mode=object_mode
            )
...
def test_chown(
    ...
    with t.make_program_and_run(
        ...
        group=caller_group,
        umask=0o022,
        runner="export NEWOWNER=$(id -u newowner); <>",
    ) as child:
...
def test_unlink_file(
    ...
    for i, case in enumerate(cases):
        ...
        t.make_file(
            f"/obj{i}/f", owner=object_user, group=object_group, mode=object_mode
        )
...
def test_unlink_link(
    ...
    for i, case in enumerate(cases):
        ...
        t.make_dir(f"/dir{i}", owner=object_user, group=object_group, mode=object_mode)
        t.make_file(
            f"/original{i}", owner=object_user, group=object_group, mode=object_mode
        )
        t.add_setup(f"echo AAAAA > /original{i}")
        t.make_file(
            f"/dir{i}/link", owner=object_user, group=object_group, mode=object_mode
        )
...
def test_link(
    ...
    for i, case in enumerate(cases):
        ...
        t.make_dir(f"/dir{i}", owner=object_user, group=object_group, mode=object_mode)
        t.make_file(
            f"/orig{i}", owner=object_user, group=object_group, mode=object_mode
        )
        t.add_setup(f"setfacl -m {','.join(acl_entries)} /dir{i}")
...
"""
    ...
    for i, case in enumerate(cases):
        _, acl_entries, object_user, object_group = case
        t.make_file(
            f"/tst_prog{i + 1}", owner=object_user, group=object_group, mode=object_mode
        )
    ...
    with t.make_program_and_run(
        ...
        argv = child.bound_value_as_charparray(
            prefix="argv", init=["/tst_prog0", str(len(cases))]
        )
