...
class TraceTranslator:
    def __init__(
        ...
        *,
        ...
        self._model_trace = EventsBuilder(
    ...
    def add_init_user(
        self, *, uid: int, primary_gid: int, supplementary_gids: list[int]
    ):

        self._model_trace.create_user(uid, [primary_gid] + supplementary_gids)

    def add_init_folder(
        self, *, path: str, dev: int, ino: int, uid: int, gid: int, perms: int
    ):
        ...
        self._model_trace.chmod(
            path, perms, parent, folder, perms, 0, 0, skip_coverage=True
        )
    ...
    def add_init_file_or_link(
        ...
        if oldpath is not None:
            ...
            self._model_trace.link(
                oldpath, path, oldParent, file, parent, 0, 0, skip_coverage=True
            )
        else:
            setgid_flag = self.mediator_state.do_stat(parent).st_mode & S_ISGID
            creat_gid = 0
            ...
            self._model_trace.chmod(
                path, perms, parent, file, perms, 0, 0, skip_coverage=True
            )
    ...
    def set_init_acl(self, *, data: list[tuple[str, list[str]]]):
        ...
        for ino, perms, acl in acls():
            ...
            for line in acl:
                macl = fullmatch(
                    r"(user|group|mask|other):([^:]*):.*([-r][-w][-x])", line
                )
                if not macl:
                    continue
                if macl[1] == "user":
        ...
        self._model_trace.set_acl(
            userACL, groupACL, groupObjACL, maskACL, dacPermissions
        )
    ...
    def openat(
        ...
        if dfd == -100:
            ...
            cwd_path = self.mediator_state.get_path(
                self.mediator_state.get_ino_of_fd(ProcFD(pid, dfd))
            )
        ...
        if self.mediator_state.do_exists(abspath):
            ...
            self._model_trace.openat_exists(
                dfd, pathname, flags, parent, file, cwd, pid, retval
            )
        else:
            if dev is None:
                dev = parent.dev
            file = Inode(dev, ino) if ino is not None else None
            self._model_trace.openat_create(
                dfd,
                pathname,
                flags,
                mode,
    ...
    def chmod(
        self, pathname: str, mode: int, pid: int, perms: Optional[int], retval: int
    ):
    ...
    def fchmod(self, fd: int, mode: int, pid: int, perms: Optional[int], retval: int):
    ...
    def chown(
        ...
        self._model_trace.chown(
            pathname, owner, group, pre_uid, pre_gid, parent, file, perms, pid, retval
        )
    ...
    def setxattr(
        ...
        file = self.mediator_state.get_ino(abspath)
        self._model_trace.setxattr(
            pathname, name, value, size, flags, parent, file, pid, retval
        )
    ...
    def set_file_exec_label(
        self, pathname: str, pid: int, smack_label: str, retval: int
    ):
        abspath = self.mediator_state.normalize(pathname, pid)
        file = self.mediator_state.get_ino(abspath)
    ...
    def link(self, oldname: str, newname: str, pid: int, retval: int):
        ...
        self._model_trace.link(
            oldname, newname, oldParent, file, newParent, pid, retval
        )
    ...
    def symlink(
        ...
        if dev is None:
            dev = parent.dev
        file = Inode(dev, ino) if ino is not None else None
        self._model_trace.symlink(
            target, linkpath, target_parent, parent, file, pid, retval
        )
    ...
    def execve(self, pathname: str, pid: int, retval: int, smack_exec: str):
        ...
        self._model_trace.execve(
            pathname, argv, envp, fds, parent, exeFile, pid, retval, smack_exec
        )
    ...
    def exit(self, pid: int, retval: int):
        ...
        fds = [
            ...
            for p, fd in self.mediator_state.fd2ino
            if p == pid
            if all(p == pid for (p, pfd) in self.mediator_state.fd2ino if pfd == fd)
        ]
