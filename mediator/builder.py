...
class DataTranslator:
    def __init__(
        self, *, m: Machine, root_dev: int, root_ino: int, root_uid: int, root_gid: int
    ):
        self.model_files = defaultdict[Inode, m.FilesItem](
            lambda: carrier_set_item(m, m.FilesItem)
        )  # implementation (#dev, #inode) to name of variable
        self.model_strings = defaultdict[str, m.StringsItem](
            lambda: carrier_set_item(m, m.StringsItem)
        )
        self.model_data = defaultdict[bytes, m.DataItem](
            lambda: carrier_set_item(m, m.DataItem)
        )
        self.model_users = defaultdict[int, m.UsersItem](
            lambda: carrier_set_item(m, m.UsersItem)
        )  # implementation to model
        self.model_groups = defaultdict[int, m.GroupsItem](
            lambda: carrier_set_item(m, m.GroupsItem)
        )  # implementation to model
        self.model_procs = defaultdict[int, m.ProcsItem](
            lambda: carrier_set_item(m, m.ProcsItem)
        )  # implementation to model
        self.model_fds = defaultdict[tuple[int, int], m.FileDescriptorsExtendedItem](
            lambda: carrier_set_item(m, m.FileDescriptorsExtendedItem)
        )  # implementation (pid, fd) to model
        ...
        self.model_fds[(0, 0)] = (
            m.AT_FDCWD
        )  # 0-th item in carrier set FILE_DESCRIPTORS_EXTENDED is AT_FDCWD
...
class EventsBuilder:
    def __init__(
        self,
        *,
        model_trace: ModelTraceConsumer,
        m: Machine,
        root_dev: int,
        root_ino: int,
        root_uid: int,
    ...
    def translate_mode(self, mode: int):
        ...
        assert (
            0 not in self._Modes
        )  # otherwise the same logics as in translate_file_flags is needed
        return frozenset(
            (model_bit for mask, model_bit in self._Modes.items() if (mode & mask) != 0)
        )
    ...
    def translate_file_flags(self, flags: int):
        ...
        model_flags = [e] + [
            flag_bit
            for f, flag_bit in self._FileFlags.items()
    ...
    def translate_xattr_flags(self, flags: int):
        ...
        return frozenset(
            (flag_bit for f, flag_bit in self._XattrFlags.items() if (flags & f) != 0)
        )
    ...
    def creat(
        ...
        self._model_trace.add(
            ...
            _name=self.translate_string(basename(path)),
            _flags=frozenset(
                (self._machine.O_CREAT, self._machine.O_TRUNC, self._machine.O_WRONLY),
            ),
            _mode=self.translate_mode(mode),
            _fd=self.translate_fd(
                (
    ...
    def set_file_label(
        self, file: Inode, smack_label: str, retval: int, skip_coverage: bool = False
    ):
        self._model_trace.add(
            set_file_label,
            _file=self.translate_inode(file),
    ...
    def execve(
        ...
        self._model_trace.add(
            ...
            _procNewLabel=self.translate_string(smack_exec)
            if smack_exec is not None
            else None,
            expected=retval >= 0,
            skip_coverage=skip_coverage,
        )
    ...
    def exit(
        self, fds: Sequence[ProcFD], proc: int, retval: int, skip_coverage: bool = False
    ):
        self._model_trace.add(
            exit,
            _proc=self.translate_proc(proc),
    ...
    def login(
        ...
        *,
        ...
        self._model_trace.add(
            ...
            _procLabel=self.translate_string(smack_label)
            if smack_label is not None
            else None,
            expected=True,
            skip_coverage=False,
    ...
    def translate_userACL(
        self, userACL: list[tuple[Inode, int, int]]
    ):  # (path, user) |-> '[r|-][w|-][x-]'
        return frozenset(
            (
                (self.translate_inode(inode), self.translate_user(uid)),
                self.translate_mode(perms),
            )
            for inode, uid, perms in userACL
        )

    def translate_groupACL(
        self, groupACL: list[tuple[Inode, int, int]]
    ):  # (path, group) |-> '[r|-][w|-][x-]'
        return frozenset(
            (
                (self.translate_inode(inode), self.translate_group(gid)),
                self.translate_mode(perms),
            )
            for inode, gid, perms in groupACL
        )

    def translate_groupObjACL(
        self, groupObjACL: list[tuple[Inode, int]]
    ):  # path |-> '[r|-][w|-][x-]'
        return frozenset(
            (
                (
                    self._machine.ROOT,
                    self.translate_mode(0o070),
                ),
    ...
    def translate_maskACL(
        self, maskACL: list[tuple[Inode, int]]
    ):  # path |-> '[r|-][w|-][x-]'
        return frozenset(
            (self.translate_inode(inode), self.translate_mode(perms))
