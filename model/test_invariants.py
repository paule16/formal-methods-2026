...
def test_FileParents_type(m: Machine):
    assert m.FileParents in total_relations(
        (m.Files - frozenset((m.ROOT,))), cartesian_product(m.Folders, m.STRINGS)
    )
...
def test_FileLink_type(m: Machine):
    assert m.FileLink in total_functions(
        m.SymLinks, cartesian_product(m.FILES, m.STRINGS)
    )
...
def test_UserACL_type(m: Machine):
    assert m.UserACL in partial_functions(
        cartesian_product(m.Files, m.Users), powerset(m.USER_PERMISSIONS)
    )
...
def test_GroupACL_type(m: Machine):
    assert m.GroupACL in partial_functions(
        cartesian_product(m.Files, m.Groups), powerset(m.GROUP_PERMISSIONS)
    )
...
def test_ProcUmask_type(m: Machine):
    assert m.ProcUmask in total_functions(
        m.Procs, powerset((m.PERMISSIONS - m.FILE_MODES))
    )
...
def test_FileXattrs_type(m: Machine):
    assert m.FileXattrs in total_functions(
        m.Files, partial_functions(m.STRINGS, m.DATA)
    )
...
def test_PathToRoot2(m: Machine):
    assert not any(
        True
        for f in m.Folders
        for p in relation_domain(relation_image(m.FileParents, frozenset((f,))))
        if not (
            function_value(m.PathToRoot, f)
            == (function_value(m.PathToRoot, p) | frozenset((p,)))
        )
    )
...
def test_PathToRoot3(m: Machine):
    assert not any(
        True
        for f in m.Folders
        for p in function_value(m.PathToRoot, f)
        if not (function_value(m.PathToRoot, p) <= function_value(m.PathToRoot, f))
    )
...
def test_PathToRoot4(m: Machine):
    assert not any(
        ...
        if not (
            any(
                True
                for c in (function_value(m.PathToRoot, f) | frozenset((f,)))
                if p in relation_domain(relation_image(m.FileParents, frozenset((c,))))
            )
        )
    )
...
def test_ProcEXE1(m: Machine):
    assert not any(
        True for p in m.Procs if not (function_value(m.ProcEXE, p) not in m.Folders)
    )
...
def test_ProcFDs1(m: Machine):
    assert not any(
        True
        for p in m.Procs
        if not (len(relation_image(m.ProcFDs, frozenset((p,)))) <= m.PROC_FILE_LIMIT)
    )
...
def test_ProcFDs3(m: Machine):
    assert not any(
        True
        for p in m.Procs
        for (key, fd) in m.ProcFDs
        if key == p
        if function_value(m.FDFile, fd) in m.Folders
        if m.O_PATH not in function_value(m.FDFlags, fd)
        if not (
            m.O_WRONLY not in function_value(m.FDFlags, fd)
            and m.O_RDWR not in function_value(m.FDFlags, fd)
        )
    )
...
def test_FileParents1(m: Machine):
    assert not any(
        True
        for f in (m.Folders - frozenset((m.ROOT,)))
        if not (
            any(
                True
                for p in m.FILES
                for n in m.STRINGS
                if relation_image(m.FileParents, frozenset((f,)))
                == frozenset(((p, n),))
            )
        )
    )
...
def test_ACL1(m: Machine):
    assert not any(
        True
        for (f, u) in relation_domain(m.UserACL)
        if not (f in relation_domain(m.MaskACL))
    )
...
def test_ACL2(m: Machine):
    assert not any(
        True
        for (f, g) in relation_domain(m.GroupACL)
        if not (f in relation_domain(m.MaskACL))
    )
...
def test_ACL3(m: Machine):
    assert not any(
        True
        for f in relation_domain(m.MaskACL)
        if not (
            function_value(m.MaskACL, f)
            == (function_value(m.DACPermissions, f) & m.GROUP_PERMISSIONS)
        )
    )
...
def test_ACL4(m: Machine):
    assert not any(
        True
        for f in m.Files
        if f not in relation_domain(m.MaskACL)
        if not (
            f in relation_domain(m.GroupObjACL)
            and function_value(m.GroupObjACL, f)
            == (function_value(m.DACPermissions, f) & m.GROUP_PERMISSIONS)
        )
    )
...
def test_ProcCwd1(m: Machine):
    assert not any(
        True
        for p in m.Procs
        if function_value(m.ProcCwd, p) in m.Files
        if not (function_value(m.ProcCwd, p) in m.Folders)
    )
...
def test_FDNumber(m: Machine):
    assert not any(
        ...
        if not (
            not any(
                True
                for fd1 in relation_image(m.ProcFDs, frozenset((p,)))
                for fd2 in relation_image(m.ProcFDs, frozenset((p,)))
                if not (
                    not (fd1 != fd2)
                    or (
                        function_value(m.FDNumber, fd1)
                        != function_value(m.FDNumber, fd2)
                    )
                )
            )
        )
...
def test_inv3(m: Machine):
    assert m.SmackRules <= cartesian_product(
        cartesian_product(m.STRINGS, m.STRINGS), m.ACCESSES
    )
