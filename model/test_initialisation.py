...
def test_act10(m: Machine):
    assert (m.ProcFDs) == (
        frozenset[tuple[Machine.ProcsItem, Machine.FileDescriptorsExtendedItem]]()
    )
...
def test_act11(m: Machine):
    assert (m.FDFlags) == (
        frozenset[tuple[Machine.FileDescriptorsExtendedItem, frozenset[int]]]()
    )
...
def test_act12(m: Machine):
    assert (m.FDFile) == (
        frozenset[tuple[Machine.FileDescriptorsExtendedItem, Machine.FilesItem]]()
    )
...
def test_act13(m: Machine):
    assert (m.FDNumber) == (
        frozenset[tuple[Machine.FileDescriptorsExtendedItem, int]]()
    )


# @act14: DACPermissions ≔ {ROOT ↦ DEF_FOLDER_PERMS, INIT_EXE ↦ DEF_FILE_PERMS}
def test_act14(m: Machine):
    assert (m.DACPermissions) == (
        frozenset(((m.ROOT, m.DEF_FOLDER_PERMS), (m.INIT_EXE, m.DEF_FILE_PERMS)))
    )
...
def test_act18(m: Machine):
    assert (m.MaskACL) == (
        frozenset[tuple[Machine.FilesItem, frozenset[Machine.PermissionsItem]]]()
    )
...
def test_act19(m: Machine):
    assert (m.FileUser) == (
        frozenset(((m.ROOT, m.ROOT_USER), (m.INIT_EXE, m.ROOT_USER)))
    )
...
def test_act20(m: Machine):
    assert (m.FileGroup) == (
        frozenset(((m.ROOT, m.ROOT_GROUP), (m.INIT_EXE, m.ROOT_GROUP)))
    )
...
def test_act23(m: Machine):
    assert (m.ProcUmask) == (
        frozenset(((m.INIT, frozenset[Machine.PermissionsItem]()),))
    )
...
def test_act24(m: Machine):
    assert (m.FileXattrs) == (
        frozenset(
            (
                (m.ROOT, frozenset[tuple[Machine.StringsItem, Machine.DataItem]]()),
                (m.INIT_EXE, frozenset[tuple[Machine.StringsItem, Machine.DataItem]]()),
            )
        )
    )
...
def test_act32(m: Machine):
    assert (m.UserCaps) == (
        frozenset(((m.ROOT_USER, frozenset[Machine.CapabilitiesItem]()),))
    )
...
def test_act34(m: Machine):
    assert (m.FileLabel) == (
        frozenset(((m.ROOT, m.ROOT_LABEL), (m.INIT_EXE, m.INIT_EXE_LABEL)))
    )
...
def test_act36(m: Machine):
    assert (m.FileExecLabel) == (
        frozenset[tuple[Machine.FilesItem, Machine.StringsItem]]()
    )
