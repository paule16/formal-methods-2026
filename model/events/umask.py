...
def umask(
    ...
    _mask: frozenset[Machine.PermissionsItem] | None,
) -> Event:
    ...
    _grd2 = Guard("grd2", lambda _: _(2, (~mask) <= (m.PERMISSIONS - m.FILE_MODES)))

    _act1 = Action(
        "act1",
        m,
        "ProcUmask",
        lambda: override_relation(m.ProcUmask, frozenset((((~proc), (~mask)),))),
    )
