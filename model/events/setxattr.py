...
def setxattr(
    ...
    _size: int | None,
    _flags: frozenset[Machine.XattrFlagsItem] | None,
) -> Event:
    ...
    _grd2 = Guard(
        "grd2", lambda _: _(2, ((~file), ((~parent), (~fileName))) in m.FileParents)
    )

    _grd3 = Guard("grd3", lambda _: _(3, (~flags) <= m.XATTR_FLAGS))

    _grd4 = Guard(
        "grd4",
        lambda _: (
            not (_(4, m.XATTR_CREATE in (~flags)))
            or (
                _(
                    5,
                    (~name)
                    not in relation_domain(function_value(m.FileXattrs, (~file))),
                )
            )
        ),
    )
    ...
    _grd5 = Guard(
        "grd5",
        lambda _: (
            not (_(6, m.XATTR_REPLACE in (~flags)))
            or (_(7, (~name) in relation_domain(function_value(m.FileXattrs, (~file)))))
        ),
    )

    _grd6 = Guard("grd6", lambda _: _(8, (~value) in m.DATA))

    _grd7 = Guard("grd7", lambda _: _(9, (~size) in NAT))
    ...
    _grd9 = Guard(
        "grd9",
        lambda _: (
            not (_(11, function_value(m.ProcUser, (~proc)) != m.ROOT_USER))
            or (
                (
                    (
                        (
                        ...
                        or (
                            (
                                ...
                                and (_(27, (~file) in relation_domain(m.MaskACL)))
                            )
                            and (_(28, m.GWRITE in function_value(m.MaskACL, (~file))))
                        )
                    )
                    or (
                        (
                            (
                                ...
                                and (_(30, (~file) not in relation_domain(m.MaskACL)))
                            )
                            and (
                                (
                                    _(
                        ...
                        and (
                            _(33, m.GWRITE in function_value(m.DACPermissions, (~file)))
                        )
                    )
                )
                or (
                    ...
                    and (_(41, m.OWRITE in function_value(m.DACPermissions, (~file))))
                )
            )
        ),
    )
    ...
    _grd11 = Guard(
        ...
        lambda _: (
            ...
            or (
                ...
                and (
                    (
                        (_(46, function_value(m.FileLabel, (~file)) == m.STAR))
                        or (
                            _(
                                47,
                                function_value(m.ProcLabel, (~proc))
            ...
            frozenset(
                (
                    (
                        ...
                        (
                            subtract_domain(
                                function_value(m.FileXattrs, (~file)),
                                frozenset(((~name),)),
                            )
