...
def fchownat(
    ...
    _grd3 = Guard("grd3", lambda _: _(3, (~cwd) in m.Folders))

    _grd4 = Guard(
        "grd4",
        lambda _: (
            not (_(4, (~dirfd) == m.AT_FDCWD))
            or (_(5, (~cwd) == function_value(m.ProcCwd, (~proc))))
        ),
    )
    ...
    _grd5 = Guard(
        "grd5",
        lambda _: (
            not (_(6, (~dirfd) != m.AT_FDCWD))
            or (
                (_(7, ((~proc), (~dirfd)) in m.ProcFDs))
                and (_(8, (~cwd) == function_value(m.FDFile, (~dirfd))))
            )
        ),
    )

    _grd6 = Guard(
        "grd6", lambda _: _(9, ((~file), ((~parent), (~name))) in m.FileParents)
    )

    _grd7 = Guard("grd7", lambda _: _(10, (~owner) in m.Users))

    _grd8 = Guard("grd8", lambda _: _(11, (~group) in m.Groups))

    _grd9 = Guard(
        "grd9",
        lambda _: (
            (_(12, function_value(m.ProcUser, (~proc)) == m.ROOT_USER))
            or (_(13, (~owner) == function_value(m.ProcUser, (~proc))))
        ),
    )
    ...
    _grd10 = Guard(
        "grd10",
        lambda _: (
            (_(14, function_value(m.ProcUser, (~proc)) == m.ROOT_USER))
            or (_(15, (function_value(m.ProcUser, (~proc)), (~group)) in m.UserGroups))
        ),
    )
    ...
    _grd12 = Guard(
        "grd12",
        lambda _: (
            (_(17, function_value(m.ProcUser, (~proc)) == m.ROOT_USER))
            or (
                _(
                    18,
                    function_value(m.ProcUser, (~proc))
    ...
    _grd13 = Guard(
        ...
        lambda _: (
            not (
                (_(19, (~file) not in m.Folders))
                and (_(20, m.GEXECUTE in function_value(m.DACPermissions, (~file))))
            )
            or (
                _(
                    21,
    ...
    _grd14 = Guard(
        ...
        lambda _: (
            not (
                (_(22, (~file) not in m.Folders))
                and (_(23, m.GEXECUTE not in function_value(m.DACPermissions, (~file))))
            )
            or (
                _(
                    24,
    ...
    _grd15 = Guard(
        "grd15",
        lambda _: (
            not (_(25, (~file) in m.Folders))
            or (_(26, (~perms) == function_value(m.DACPermissions, (~file))))
        ),
    )
    ...
    _grd16 = Guard(
        "grd16",
        lambda _: (
            not (_(27, function_value(m.ProcUser, (~proc)) != m.ROOT_USER))
            or (
                _(
                    28,
                    not any(
