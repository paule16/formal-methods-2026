# formal-method-2026

В репозитории была проведена верификация подсистемы Linux Smack.
Smack - это мандатная система доступа, основанная на метках.

Разработчики проекта:
1.) Сахаров Павел
2.) Званцов Матвей
3.) Должков Александр
4.) Межуев Тимофей

## Ограничения

1.) По документации Linux Smack процесс может обладать меткой *, но при разработке это не удалось воспроизвести;
2.) В архитектуре Linux Smack предполагается, что правила не могут быть удалены. Это приводит к значительному увеличению количества меток;

## Обратная связь

1.) При дебаге "неуспешные" гарды нужно доставать своими силами: отладочная печать, например.
Следует добавить логирование в библиотеку.
2.) Отсутствие документации
3.) Отсутствие параллельного запуска тестов
4.) Нестабильность системы (невозможность сгенерировать покрытие)
